    ...
    private static void MapSearch(RouteGroupBuilder customers)
    {
        customers.MapGet("/", async Task<IResult> (
            ...
            .WithDescription(
                ...
                + "shorter than three characters is refused with 400 "
                + "`customers.search-term-too-short` rather than answered, so an empty page means "
                + "that nobody matched and only that. A term that is absent or empty is not that "
                + "refusal: it is not a question, and it is answered with an empty page.")
