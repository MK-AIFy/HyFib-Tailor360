...
| --- | --- | --- | --- | --- |
| **DOD-OD-01** | The minimum number of approving reviews on a pull request into `main`, and whether the technical reviewer may approve their own work during a single-maintainer period | Branch protection configuration in #22 and #59 | Business owner, with the technical reviewer | **Decided — 2026-09-19**: **zero** required approving reviews while there is one maintainer, with bypass **off** and every status check required, so the machinery is enforced even though a second reader does not exist. The author records an explicit self-review in the pull request. It becomes **one** the day a second maintainer has write access. Recorded in [`branch-protection.md`](branch-protection.md) as **BP-OD-01** |
