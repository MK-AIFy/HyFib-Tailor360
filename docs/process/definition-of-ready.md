...
| **DOR-05** | **Threat-model coverage.** The flow the issue touches is covered by a threat model, or — before #56a merges — is listed in the threat-model backlog with the issue that will cover it, **the wave that dates that issue**, and the interim controls that apply. The wave and its exit gate are the commitment: this project schedules by wave rather than by calendar date, and a date invented for a table is a date nobody is held to | From #56a onwards: the threat model file is named in the issue and its mapped controls are listed. Before #56a: the flow appears in the backlog list of [`../security/README.md`](../security/README.md) section 3.2 with its covering issue and wave | [`../security/README.md`](../security/README.md) section 3.2, then the `docs/security/` threat models (#56a) |
