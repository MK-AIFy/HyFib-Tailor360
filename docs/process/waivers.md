...
| --- | --- | --- | --- | --- |
| The rendered invoices, credit notes and debit notes are not tagged PDFs (no PDF/UA structure): QuestPDF, chosen by ADR-0014, writes the title, language and subject metadata and embeds the fonts, but produces no accessibility tags, so the document accessibility check #155 asks for cannot pass on the document alone. The figures on the document are also available to assistive technology through the API and the screens that render the same invoice | [`../adr/0014-document-rendering-and-object-storage.md`](../adr/0014-document-rendering-and-object-storage.md) section 4 | Technical reviewer | An issue against #155, to be opened when the accessibility gate for documents (RG-06's document profile) is defined by #52 | RG-06 accessibility scan |
| No accountant has yet reviewed an invoice and a credit note rendered from synthetic data (#155 acceptance criterion). The rendering is deterministic and its figures are asserted against the stored calculation by the worker and by the tests, but the layout and the statutory wording are an accountant's to sign | [`../adr/0014-document-rendering-and-object-storage.md`](../adr/0014-document-rendering-and-object-storage.md) section 4 | Business owner, with the accountant | The review itself, recorded on #42 when done | RG-14 financial evidence |
