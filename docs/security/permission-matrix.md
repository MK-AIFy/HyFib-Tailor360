...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `billing.approve_dispatch_exception` | Billing | branch | yes | yes | yes | `owner` | #164 | Single-use, approved by someone other than the dispatcher. The plan's interim position is the Owner only (`raci.md` footnote (15), XQ-02). Also gates the narrow read of the order's balance the approval screen needs (#220), rather than granting the Owner `payments.record` |
...
| `payments.approve_reconciliation` | Billing | branch | yes | yes | yes | `owner`, `branch_manager` | #172 | Above threshold, by a different user from the cashier who closed the session; step-up and a reason (INV-CSH-04). Also gates the narrow read of the session being approved (#220), rather than granting the Owner `payments.session` |
...
| `GET` | `/api/v1/billing/receipts/{receiptId}/document` | permission | `billing.print_receipt` | current-branch | `billing.receipt` | `billing.receipt.downloaded` | Streams the rendered receipt, re-authorised in branch scope on every call, the access audited against the receipt; not available until the worker has rendered it |
| `POST` | `/api/v1/billing/receipts/{receiptId}/print` | permission | `billing.print_receipt` | current-branch | `billing.receipt` | `billing.receipt.printed` | Sends the rendered receipt to the branch's print queue on the receipt roll, one to five copies. Idempotent |
| `GET` | `/api/v1/billing/receipts/barcode/{payload}` | permission | `billing.print_receipt` | current-branch | — | — | Resolves an `R-` payload to the receipt it was printed on, for the issuing branch alone; another branch's, a wrong check character, an `I-` payload and an unknown payload read alike as 404. No resource scope: the parameter is an opaque payload, parsed before anything can be named |
...
| `GET` | `/api/v1/billing/cashier-sessions/{sessionId}/reconciliation` | permission | `payments.approve_reconciliation` | current-branch | `billing.cashier_session` | — | Reads the closed session a reconciliation approval is about to decide on, exactly `GetCashierSession`'s own record; added under `payments.approve_reconciliation` alone (#220) because the Owner who approves does not hold `payments.session`, and reaches no more than the approve route already reaches — any closed session at the caller's own branch. Step-up, because the catalogue's flag is per permission, not per route. Another branch's reads as 404 |
| `POST` | `/api/v1/billing/cashier-sessions/{sessionId}/reconciliation/approve` | permission | `payments.approve_reconciliation` | current-branch | `billing.cashier_session` | `payments.approve_reconciliation` | Approves a closed session's variance, by someone other than the cashier who closed it (INV-CSH-04); refused where no variance on the batch was ever beyond the threshold, and refused twice. Step-up and a reason. Idempotent. The batch itself is read back on the session's own payload; the session itself has the row above |
| `GET` | `/api/v1/billing/orders/{orderId}/dispatch-exception-balance` | permission | `billing.approve_dispatch_exception` | current-branch | `billing.order` | — | Reads what an order still owes, for the approver about to set a dispatch exception's maximum outstanding allowance; exactly `GetOrderBalance`'s own figures, added under `billing.approve_dispatch_exception` alone (#220) because the Owner who approves does not hold `payments.record`, and reaches no more than the approve route already reaches — an order Billing knows at the caller's own branch. Step-up, because the catalogue's flag is per permission, not per route. An order Billing has not heard of, or one at another branch, reads as 404 |
| `POST` | `/api/v1/billing/dispatch-exceptions` | permission | `billing.approve_dispatch_exception` | current-branch | — | `billing.approve_dispatch_exception` | Approves a single-use dispatch exception for named jobs of an order, bound to a maximum outstanding amount, the dispatch policy version and an expiry of at most 72 hours (EX-10); refused where the order is not Billing's, is at another branch, or names a job that is not a live job of it. Consumed exactly once through `IDispatchEligibilityQuery`, which re-validates the balance, the job set, the policy version, the expiry and that the dispatcher differs from the approver — none of that at this route. Step-up and a reason. Idempotent. No resource scope: the exception is approved at the caller's own branch against an order the handler checks is the branch's |
