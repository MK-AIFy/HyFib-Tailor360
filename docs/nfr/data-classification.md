...
| Deletion and anonymisation | Expiry by backend retention policy. A client telemetry redaction test runs in the pull-request pipeline (#52, #58) — `ClientTelemetryEndpointTests.ABatchCarryingSixSentinelValuesIsAcceptedAndNoneOfThemReachesTheEmittedLogRecord` posts a batch carrying a customer name, a phone number, a measurement value, a full stack trace, a URL with a query string and a route parameter value, and asserts none of the six sentinels reaches the emitted log record; `ClientTelemetryAllowlistTests.AnArbitraryAttributeBagNeverSurvivesWithAnAttributeNameOutsideTheEventTypesDeclaredSet` is the companion property test proving no attribute bag, however arbitrary, can produce a surviving name outside an event type's declared set |
