...
| **A11Y-DP-02** — read with the right phonetics; a Tamil name in Tamil | **Fail** | The document declares one document-wide `/Lang` of `en-IN` and **no per-span language**, so a screen reader cannot change voice mid-line and a Tamil customer name is pronounced with English phonetics. The name *renders* correctly — asserted in `WritesTheAccountantsSamplePack` — so this is pronunciation, not glyphs. QuestPDF 2026.8.0 emits no tagged structure. **Closed by #512** |
...
| **A11Y-DP-04** — reading order matches visual order | **Fail** | The extracted order *is* the visual order, and the figure assertions depend on it, so the practical answer is good. But without a structure tree the order is **inferred from geometry by the reader** rather than declared by the document, so a reader that infers differently is not contradicted. **Closed by #512** |
| **A11Y-DP-05** — tables carry header cells | **Fail** | The assertable half passes: the column headings are printed once, above their rows, and `DocumentAdapterTests` pins that. There is **no table-header role**, because the document carries no structure tree, so a reader cannot announce "Taxable value" when it reaches a cell. **Closed by #512** |
