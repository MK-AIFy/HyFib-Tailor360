...
| 2 | **The job that signs and promotes the web, worker and CLI images** | CI checks that all four Docker targets build, including the CLI image and client assets. It does not publish signed digests, so `TAILOR360_*_IMAGE` still has no release image to point at and staging cannot run yet | #22 continuation, or #59 |
