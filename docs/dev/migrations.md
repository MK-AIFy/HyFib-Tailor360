...
| `20260924083859_InitialOrdersSchema` | `orders` | Expand — initial | #32 draft intake slice | Applied to local development PostgreSQL and to a disposable empty database. Creates `order_drafts`, `draft_garments`, and the module inbox and outbox. On the disposable database, `Down` removed the draft table and re-application restored it; the disposable database was then dropped. `dotnet ef migrations has-pending-model-changes` reports none. This schema stores unpriced intake drafts only; it does not confirm orders or issue invoices. |
