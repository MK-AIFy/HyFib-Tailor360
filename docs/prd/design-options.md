...
| **DR-03** | note | `front_neck = HIGH_NECK` → "Closed neck front and back: confirm the opening — back hooks or a side zip. Applies when the back neck is also high; check the back-neck selection before printing the summary." | Seeded as an exclusion and re-typed here. The neckline height is not what opens the garment; the closure is, and this category already offers back hooks, a back zip and a side zip. A high neck front and back is an ordinary weekly order, especially for temple wear, so it must not be refused at confirmation (**OD-DES-04**). **Corrected at seed (#139):** the rule was written as two conditions joined by "with" (`front_neck = HIGH_NECK` with `back_neck = HIGH_NECK`), a compound the operand grammar of Section 4 does not define — every form there is over one group — and the merged rule model (#137) carries exactly one antecedent per rule. The seed anchors the note on `front_neck` alone and folds the back-neck qualification into the printed text, which a non-blocking note can safely do; see DR-14 for the same correction |
| **DR-04** | excludes | `sleeve_style = SLEEVELESS` excludes `finish includes LACE_EDGE` | Seeded as a shop preference, not a physical law — listed here as the worked example of a rule the Owner may well delete at review (**OD-DES-04**). **Corrected at seed (#139):** `finish` is seeded as a *multiple*-selection group (its own groups table says so), and Section 4 reserves `=` for a single-selection group's value; `includes` is the form a multiple-selection group takes. The statement above is corrected from `finish = LACE_EDGE`, which named the right option but the wrong form |
...
**Seed note (#139).** The nine copied groups' own option rows are not restated in the options table above,
unlike the partial reuse in 9.3–9.5, whose rows say explicitly "seeded here with no price or time impact". This
document reads that absence, together with "the same codes and the same meanings", as the copies keeping
`BLOUSE_PATTERN`'s own price-list item codes and day impacts (`PI_BLOUSE_HIGH_NECK`, `PI_BLOUSE_FULL_SLEEVE` and
the rest) rather than losing them — the seed is built that way. Confirm this reading at the **OD-10** review.
...
| **DR-14** | note | `bottom_style = CHURIDAR` → "Cut the churidar lining to the same ankle length and gather both layers together; allow the extra length. Applies when the bottom is lined." | Seeded as an exclusion and re-typed here. Churidars in net, georgette and light silk are lined as a matter of course; the lining is cut to the same pattern and gathered with the shell, so the gathering at the ankle is what the lining follows rather than a reason it cannot exist (**OD-DES-04**). **Corrected at seed (#139):** as DR-03, the rule joined two conditions with "with" (`bottom_style = CHURIDAR` with `lining = FULL`), which the single-antecedent rule grammar and model cannot express; the seed anchors the note on `bottom_style = CHURIDAR` and folds the lining qualification into the printed text |
