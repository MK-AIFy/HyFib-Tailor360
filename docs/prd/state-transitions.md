...
| Delivered | Alteration requested and accepted — **REASON** | In production | Branch Manager or Tailor Master (`orders.alteration_decide`) | Request recorded from staff or from feedback through `IAlterationRequests.OpenAsync(...)` — a configured reason code and the feedback identifier, never the customer free text; price and due-date decisions taken and communicated | `AlterationRequested`, `AlterationDecided`; new or reopened job linked to the original; billing adjustment intent | `orders.alteration_decide` | A rejected alteration is also recorded with its reason and communicated; it is never left silent |
