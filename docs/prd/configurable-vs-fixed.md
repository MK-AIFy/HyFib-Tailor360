...
| GST registrations per branch — GSTIN, state code | Yes, configuration data | Owner with the price-list administration permission (`billing.manage_price_lists`, decided with #145): a registration is what every invoice is printed with and what place of supply is decided against, so it is administered with the billing configuration rather than with the branch record | Yes — audited change history | Applies to documents created afterwards; posted documents keep the registration they were posted with |
