...
| 7 | **Rate limiting** | Applies the endpoint's declared rate-limit policy from the catalogue — the anonymous authentication and MFA-challenge policies, the customer-link and callback policies, telemetry ingest, the scan burst sized for offline-queue replay, heavy exports, and the per-user and per-IP defaults. Every endpoint declares exactly one policy, asserted by an architecture test; the limiter keys on the client address resolved in stage 1 | `429` problem details |
