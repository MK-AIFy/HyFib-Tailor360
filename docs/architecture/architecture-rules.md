...
| | |
| --- | --- |
| **Assertion** | Every schema declared in a module's `Infrastructure` project equals that module's own schema name — `identity`, `customers`, `catalog`, `media`, `orders`, `custody`, `inventory`, `billing`, `reporting`, `notifications`, `integration` — and no other, **with exactly one named exception**: a module's own `Infrastructure` project may additionally map the single table `platform.audit_events`, and only that table, through the shared `AuditEventMapping.Configure` helper, so that its own `AuditWriter<TContext>`-based port can track an entry on that context's own change tracker and save it by the same `SaveChangesAsync` call as the change it describes (ADR-0015, issue #179). No other table, no other schema and no other call site may cross this way. |
| **Rationale** | Schema-per-module (D3, ADR-0004) is what makes forbidden cross-module data access visible to a test and enforceable by a database role. Two modules mapping one table is the failure that turns the monolith back into a single tangled schema, and it defeats the append-only triggers that protect ledger entries, scan events and posted invoices, because a second owner can map the same rows with different expectations. The one named exception does not reopen that hazard: ownership, migration rights and the application role's write privilege on `audit_events` stay exactly where they were — `Tailor360.Platform.Persistence` and `t360_migrator` alone create, alter and hash-chain the table, every other context maps it with `ExcludeFromMigrations()`, and only same-transaction enlistment of an insert is added. ADR-0004 already anticipated this exact crossing (order confirmation's barcode identity and invoice posting's audit event, Section 1) and left its mechanism to be decided when a module first needed it; ADR-0015 is that decision, made after issue #179 found that the interceptor ADR-0004 sketched had never been built and G-2 was being broken in practice. |
| **Allowed exceptions** | Two, and they are checked differently. First, the `platform` schema as a whole is owned by `Tailor360.Platform.Persistence` — outbox, inbox, idempotency keys, sequences, audit events, feature flags, print jobs, job leases — which is not a module `Infrastructure` project, so *that* project is outside the scanned set entirely. Second, and narrower: a module's own `Infrastructure` project mapping `platform.audit_events` specifically through `AuditEventMapping.Configure`, per the assertion above — this one *is* inside the scanned set, and the detector checks it by name rather than by ignoring it. Reporting reads other modules' data only from its own projections, never from their tables. |
| **How an exception is registered** | For any other shape, it is not: a module needing another module's data adds a read contract or subscribes to an event, and `ModuleContextRegistry` additionally refuses at runtime to register two contexts for one schema, so the rule also fails fast outside the test suite. The one named exception above is already registered, in this row and in ADR-0015; a module adopting it calls the existing `AuditEventMapping.Configure(modelBuilder)` from its own `OnModelCreating` and nothing else — there is no second registration step, and no other helper may be added for the same purpose without amending this rule and ADR-0015 together. |
| **Test** | `SourceConventionTests.Arch005_ModulePersistenceDeclaresOnlyItsOwnSchema` in `tests/Tailor360.ArchitectureTests/SourceConventionTests.cs`. The detector reads `HasDefaultSchema("…")` declarations, and separately reads every explicit `ToTable(name, schema)` overload literal in a module's own source — a mismatch on either fails, with one exception recognised by name: a call to `AuditEventMapping.Configure` is read as declaring exactly `platform`/`audit_events`, because that is the only table the helper maps, and any other schema or table reaching outside the module's own is still a failure. The negative control proving the extended detector still catches an unnamed violation is `NegativeControlTests.Arch005DetectorCatchesAnUnsanctionedForeignSchemaMapping`. |
...
| | |
| --- | --- |
| **Assertion** | No project other than `Tailor360.Modules.Integration.Infrastructure` and the test projects references a package whose name starts with a listed vendor prefix (currently `Razorpay`, `Stripe`, `Twilio`, `SendGrid`, `AWSSDK`, `Google.Apis`, `Zoho`, `ClamAV`, `SkiaSharp`). `SkiaSharp` is a local codec library rather than a provider, listed for the same reason as a provider SDK: its native binaries and transitive dependencies belong behind `IImageProcessor` in one project, not in the domain or the hosts (#597). |
...
| **Test** | `EndpointResourceScopeTests.EveryPublishedEndpointDeclaresItsAuthorisationConsistently`, with `TheInspectorCatchesAPermissionedRouteWithAnIdentifierAndNoResourceScope` and `TheInspectorCatchesAnAssignedBranchesRouteWithAnIdentifierAndNoResourceScope` as its negative controls and `TheInspectorAcceptsTheThreeWaysARouteNeedsNoResourceScope` as the converse. The rule governed no published route when this row was written; Billing, Orders and Catalog now each declare `ScopedToResource` on a shipped endpoint, and Customers declares `TouchesNoBranchOwnedResource` on the routes AS-05 of [`../security/threat-models/authorisation.md`](../security/threat-models/authorisation.md) explains, so the controls are no longer the rule's whole evidence. |
...
| --- | --- | --- | --- |
| ARCH-007 | The three orchestrator health probes under `/health/` (`/health/live`, `/health/ready`, `/health/startup`) | Health-probe metadata recognised by the test | #20, plan Section 4.4 |
| ARCH-007 | `/health/detail` on the worker host, which composes no authentication at all | `AllowAnonymousWithJustification` in `Tailor360.Worker`'s `Program.cs` | #447 |
...
| ARCH-023 | A route whose parameter names nothing branch-owned | `TouchesNoBranchOwnedResource` on the endpoint | #24 for the mechanism; #26 (Customers) is the first module to invoke it, on the routes [`../security/threat-models/authorisation.md`](../security/threat-models/authorisation.md) AS-05 records |
