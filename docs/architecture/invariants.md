...
| G-2 | The audit event for a mutation is written **in the same transaction** as the mutation, by the `SaveChanges` interceptor and the `[Audited("module.action")]` endpoint filter. A committed change with no audit row is impossible. | `Platform.Persistence`, architecture test |
