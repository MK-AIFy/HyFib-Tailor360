...
| `IImageProcessor` | Decode, validate, strip and derive an uploaded image | #597 | SkiaSharp; always the adapter — a pure library with no endpoint, so like `IPdfRenderer` and `IBarcodeRenderer`, and unlike `IMalwareScanner`, it has no unconfigured state and no fake |
